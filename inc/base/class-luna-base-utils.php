...
abstract class Luna_Base_Utils {
	...
	public function image( $id_or_url, $size = 'large', $size_retina = false, $class = '', $echo = true ) {
		...
		if ( is_admin() ) {
			...
			$atts['src'] = 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' . $size['width'] . ' ' . $size['height'] . '"%3E%3C/svg%3E';
		...
		<figure class="<?php echo esc_attr( $class ); ?>">
			<picture>
				<?php if ( is_admin() ) : ?>
					...
					<source srcset="<?php echo esc_url( $tablet_src ); ?>" media="(min-width: 375px)" />
					<img src="<?php echo esc_url( $mobile_src ); ?>" alt="<?php echo esc_attr( $image_alt ); ?>" width="<?php echo esc_attr( $width ); ?>" height="<?php echo esc_attr( $height ); ?>" class="<?php echo esc_attr( $class . '__image' ); ?>" />
				<?php else : ?>
					<source data-srcset="<?php echo esc_url( $desktop_src ); ?>" media="(min-width: 768px)" />
					<source data-srcset="<?php echo esc_url( $tablet_src ); ?>" media="(min-width: 375px)" />
					<img src="<?php echo esc_attr( $placeholder_src ); ?>" data-src="<?php echo  esc_url( $mobile_src ); ?>" alt="<?php echo esc_attr( $image_alt ); ?>" width="<?php echo esc_attr( $width ); ?>" height="<?php echo esc_attr( $height ); ?>" class="<?php echo esc_attr( $class . '__image lazy' ); ?>" />
				<?php endif; ?>
			</picture>
		</figure>
		<?php
